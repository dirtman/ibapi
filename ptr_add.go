...
func addPTR(invokedAs []string) error {
	...
	for nameData, state := range states {
		...
		if len(states[nameData].records) != 0 {
			if state.records[0].PtrdName == name {
				conflict += sep + "PTR record with same name"
			} else {
				conflict += sep + "PTR record with same IP"
			}
			sep = ", "
		}
		...
		if check && len(statesA[nameData].records) != 0 {
			for _, record := range statesA[nameData].records {
				if record.Ipv4Addr != data {
					conflict += sep + "related A record with different IP"
					break
				} else if record.Name != name {
					conflict += sep + "related A record with different name"
					break
				}
			}
		}
