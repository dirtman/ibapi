...
The delete command is used to delete Infoblox A records.
To delete a single A record, a single hostname and optionally an IP address may
be provided as command line arguments.
Alternatively, a list of records to delete can be specified in a file (see --filename).

If an IP address is specified, the A record to delete must contain that IP address, else
no address will be deleted.  If no IP address is specified, the A record is deleted
...
- -f &lt;filename>, --filename=&lt;filename>:
    ...
    Specify a filename containing a list of A records to delete.
