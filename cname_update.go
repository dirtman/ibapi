...
func upateCNAME(invokedAs []string) error {
	...
	SetStringOpt("target", "t", false, "", "Update the record's target (CNAME)")
	...
	if target != "" { // Append it to the list of field/values to be updated.
		input.fields = append(input.fields, "canonical="+target)
	}

	// Query the record being updated, and check for errors.
	states := make(StatesCNAME)
	...
	if name != "" {
		...
		// Check for A records, the most common; we'll settle for a 4xx WAPI error for the rest.
		if conflict, err = checkConflict(f, check, true, true, true, true, "A"); err != nil {
			return Error("failure checking host conflicts: %v", err)
		}
	}
	if conflict != "" {
		return Error("%-*s NOT updated: %s\n", space, "CNAME("+request+")", conflict)
	}
